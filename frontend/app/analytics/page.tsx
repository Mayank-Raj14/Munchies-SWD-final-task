      ...
      {loading ? (
        ...
        <>
          <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            ].map(([label, value]) => <MarketSurface className="page-fade-in p-5" key={label}><p className="text-sm text-foreground-muted">{label}</p><p className="mt-2 text-2xl font-semibold tracking-tight text-foreground">{value}</p></MarketSurface>)}
          </section>
          <MarketSurface className="page-fade-in mt-6 p-5">
            ...
            <div className="mt-4 space-y-3">
              {Object.entries(analytics.monthlySpending).length ? Object.entries(analytics.monthlySpending).map(([month, value]) => (
                <div className="grid gap-2 sm:grid-cols-[8rem_1fr_7rem] sm:items-center" key={month}>
                  ...
                  <span className="h-3 rounded-full bg-surface-raised">
                    <span className="block h-full rounded-full bg-accent transition-[width] duration-300" style={{ width: `${Math.max(6, (value / maxMonth) * 100)}%` }} />
                  </span>
