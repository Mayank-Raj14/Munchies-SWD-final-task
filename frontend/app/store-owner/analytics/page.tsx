  ...
  return (
    <PageContainer size="wide">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <select className="rounded-lg border border-border bg-surface px-3 py-2 text-foreground" onChange={(e) => setStoreId(e.target.value)} value={storeId}>
          {stores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}
        </select>
        <input className="rounded-lg border border-border bg-surface px-3 py-2 text-foreground" min={0} onChange={(e) => setThreshold(Number(e.target.value))} type="number" value={threshold} />
      </div>
      {loading ? (
        ...
        <>
          <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            ].map(([label, value]) => <MarketSurface className="p-5" key={label}><p className="text-sm text-foreground-muted">{label}</p><p className="mt-2 text-xl font-semibold text-foreground">{value}</p></MarketSurface>)}
          </section>
          <section className="mt-6 grid gap-6 lg:grid-cols-2">
            <MarketSurface className="p-5"><h2 className="text-lg font-semibold text-foreground">Booking statistics</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{Object.entries(analytics.bookingStatistics).map(([status, count]) => <div className="rounded-lg bg-surface-raised p-3" key={status}><p className="text-sm text-foreground-muted">{status}</p><p className="text-xl font-semibold text-foreground">{count}</p></div>)}</div></MarketSurface>
            <MarketSurface className="p-5"><h2 className="text-lg font-semibold text-foreground">Low stock alerts</h2><div className="mt-4 space-y-2">{analytics.lowStockItems.length ? analytics.lowStockItems.map((item) => <div className="flex justify-between rounded-lg bg-surface-raised p-3" key={item.id}><span>{item.name}</span><strong>{item.stock}</strong></div>) : <p className="text-sm text-foreground-muted">No items at or below threshold.</p>}</div></MarketSurface>
          </section>
        </>
      ) : null}
    </PageContainer>
